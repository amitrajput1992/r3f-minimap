...
import { backgroundFragment, backgroundVertex } from "./shaders/background.ts";
...
const Frame = forwardRef((props: Props, ref: ForwardedRef<Mesh>) => {
  ...
  const additionalMeshProps: Record<string, any> = {};
  if(materialOptions.opacity === 0) {
    additionalMeshProps["renderOrder"] = 999;
  } else {
    additionalMeshProps["userData"] = {
      needsRenderOrder: true
    };
  }

  return (
    <group position={position} rotation={rotation} onClick={onClick}>
      {/* add collision: true to enable collisions on this object */}
      <mesh
        ref={ref}
        name={"Block-Frame"}
        castShadow={true}
